...
它用来在 Codex、Claude Code、Pi、Kiro 和 Gemini 之间迁移工作上下文，不需要手动复制
长 transcript，也不需要把本地 session history 上传到云端服务。
...
Codex 使用 `knowledge-transfer` skill。它适合在你不想记具体参数时，让 agent 选择
